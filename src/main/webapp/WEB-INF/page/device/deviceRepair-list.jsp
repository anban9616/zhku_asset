...
<title>设备维修类型列表</title>
</head>
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 设备维修类型管理 <span class="c-gray en">&gt;</span> 用户管理 <a
	...
	<div class="page-container">
		<div class="text-c">
			<input type="text" class="input-text" style="width: 250px" placeholder="输入设备维修类型名称" id="" name="">
			<button type="submit" class="btn btn-success" id="" name="">
				<i class="Hui-iconfont">&#xe665;</i> 搜设备维修类型
			</button>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a
				href="javascript:;" onclick="admin_add('添加设备维修类型','${path}/deviceRepair/add','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加用户</a>
		...
		<table class="table table-border table-bordered table-bg">
			<thead>
				...
				<tr class="text-c">
					...
					<th width="100">报修备注</th>
					<th width="100">维修人</th>
					<th width="100">维修日期</th>
					<th width="100">维修备注</th>
					<th width="100">状态</th>
			...
			<tbody>
				<c:forEach items="${requestScope.list }" var="item" varStatus="status">
					<tr class="text-c">
						...
						<td>${item.code }</td>
						<td>${item.damager }</td>
						<td><fmt:formatDate value="${item.damageDate }" pattern="yyyy-MM-dd" /></td>
						<td>${item.damageRemarks }</td>
						<td>${item.repairer }</td>
						...
						<td class="td-manage">
							<a title="编辑" href="javascript:;" onclick="admin_edit('设备维修类型编辑','${path}/deviceRepair/edit?id=${item.id }','800','500')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a> 
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${path}/deviceRepair/deleteOne?id='+id,
			dataType: 'json',
