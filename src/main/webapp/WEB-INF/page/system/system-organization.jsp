	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a
				href="javascript:;" onclick="organization_add('添加部门','${path}/organization/add','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加部门</a>
			...
			<tbody>
				<c:forEach items="${requestScope.list }" var="item" varStatus="status">
					<tr class="text-c">
						...
						<td class="td-manage"><a title="编辑" href="javascript:;" onclick="organization_edit('部门编辑','${path}/organization/edit?id=${item.id }','800','500')" class="ml-5"
							style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="organization_del(this,${item.id})" class="ml-5"
...
/*管理员-增加*/
function organization_add(title,url,w,h){
...
function organization_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${path}/organization/deleteOne?id='+id,
			dataType: 'json',
...
/*管理员-编辑*/
function organization_edit(title,url,w,h){
	layer_show(title,url,w,h);
}
</script>
</body>
