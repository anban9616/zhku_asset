...
<title>添加设备报废 - 设备报废管理 - H-ui.admin v3.1</title>
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" id="form-deviceOut-edit">
	<input name="id" type="hidden" value="${requestScope.deviceOut.id }"> 
	<input name="did" type="hidden" value="${requestScope.deviceOut.did }"> 
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>审核备注：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${requestScope.deviceOut.approvalRemarks }" placeholder="" id="approvalRemarks" name="approvalRemarks">
		</div>
	</div>
	<input id="status" type="hidden" value="${requestScope.deviceOut.status }">
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>状态：</label>
		<div class="formControls col-xs-8 col-sm-9 skin-minimal">
			<div class="radio-box">
				<input type="radio" id="status-1" name="status" value="1">
				<label for="status-1">申请报废</label>
			</div>
			<div class="radio-box">
				<input type="radio" id="status-2" name="status" value="2">
				<label for="status-2">确认报废</label>
			</div>
			<div class="radio-box">
				<input type="radio" id="status-3" name="status" value="3">
				<label for="status-3">拒绝报废</label>
			</div>
		</div>
	</div>
	<div class="row cl">
...
$(function(){
	...
	$(":radio").eq(status-1).attr("checked","checked");
	$('.skin-minimal input').iCheck({
		checkboxClass: 'icheckbox-blue',
		radioClass: 'iradio-blue',
		increaseArea: '20%'
	});
	$("#form-deviceOut-edit").validate({
		rules:{
			approvalRemarks:{
				required:true,
				minlength: 1,
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				async:false,
				type: 'post',
				url: "${applicationScope.path }/deviceOut/editAfter",
