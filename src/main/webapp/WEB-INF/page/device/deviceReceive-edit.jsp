...
<article class="page-container">
	<form class="form form-horizontal" id="form-deviceReceive-edit">
	<input name="id" type="hidden" value="${requestScope.deviceReceive.id }"> 
	<input name="did" type="hidden" value="${requestScope.deviceReceive.did }"> 
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>归还备注：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${requestScope.deviceReceive.rtRemarks }" placeholder="" id="rtRemarks" name="rtRemarks">
		</div>
	</div>
	<input id="status" type="hidden" value="${requestScope.deviceReceive.status }">
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>状态：</label>
		<div class="formControls col-xs-8 col-sm-9 skin-minimal">
			<div class="radio-box">
				<input type="radio" id="status-1" name="status" value="1">
				<label for="status-1">领取</label>
			</div>
			<div class="radio-box">
				<input type="radio" id="status-2" name="status" value="2">
				<label for="status-2">归还</label>
			</div>
		</div>
	</div>
	<div class="row cl">
...
$(function(){
	...
	$("#form-deviceReceive-edit").validate({
		rules:{
			rtRemarks:{
				required:true,
				minlength: 1,
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				async:false,
				type: 'post',
				url: "${applicationScope.path }/deviceReceive/editAfter",
