...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${device.code }" placeholder="" id="" name="" disabled="disabled">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>设备序列号：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${device.sno }" placeholder="" id="sno" name="sno">
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${device.original }" placeholder="" id="original" name="original">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>净残值：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${device.residual }" placeholder="" id="residual" name="residual">
		</div>
	</div>
	<!-- 购买日期 -->
	<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value='<fmt:formatDate value="${device.prodDate }"/>' placeholder="" id="prodDate" name="prodDateStr">
			</div>
	</div>
	<!-- 设备类型 -->
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> 
			...
			<select class="select" name="dtid" size="1">
				<c:forEach var="item" items="${requestScope.deviceTypeList }">
					<option value="${item.id }" ${(device.dtid eq item.id)?'selected':'' }>${item.name }</option>
				</c:forEach>
			</select>
			</span> 
		</div>
		...
		<div class="formControls col-xs-8 col-sm-9"> 
			...
			<select class="select" name="ogid" size="1">
				<c:forEach var="item" items="${requestScope.organizationList }">
					<option value="${item.id }" ${(device.ogid eq item.id)?'selected':'' }>${item.name }</option>
				</c:forEach>
			</select>
			</span> 
		</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> 
			...
			<select class="select" name="status" size="1">
				<option value="1" ${(device.status eq 1)?'selected':'' }>入库</option>
				<option value="2" ${(device.status eq 2)?'selected':'' }>出库中</option>
				<option value="3" ${(device.status eq 3)?'selected':'' }>出库</option>
				<option value="4" ${(device.status eq 4)?'selected':'' }>领用</option>
				<option value="5" ${(device.status eq 5)?'selected':'' }>报修</option>
			</select>
			</span> 
		</div>
	</div>
	...
	$("#form-admin-edit").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				async:false,
				type: 'post',
				url: "${applicationScope.path }/device/editAfter",
